	...
	class MIMEDECODE 
	{
	    ...
	    */
	    ...
	    function _parseHeaders($input)
	    {
	        ...
	        if ($input !== '') {
	            ...
	            foreach ($headers as $value) {
	                ...
	                $return[] = array(
	                                  'name'  => $hdr_name,
	                					'value' => $this->_decodeHeader($hdr_value)
		...
		//	$msg.=$this->walk(&$object,"",$main_content_type);
			//print $msg."hallo world";
			$From=$object->headers[from];
			$adresas = substr($From,stripos($From,'<')+1,stripos($From,'>')-stripos($From,'<')-1);
			$vardas = substr($From,0,stripos($From,'<'));
			$msg[]=array(
						'To'=>$object->headers[to],
						'vardas'=>$vardas,
						'adresas'=>$adresas,
						'Subject'=>$object->headers[subject],
						'Date'=>$object->headers[date],
						'Message'=>$this->walk(&$object,"",$main_content_type,"","")
						);
			return $msg;
		}

		function walk($object,$msg="",$main_content_type="",$plain="",$html="")
		{
			if(!isset($object->parts))
			{
				    ...
				    if (file_exists($temp . $name))
    				 {
     					...
     					{
     						list($pradzia, $galune) = explode(".",$name,2);
    						$name=$pradzia.'-'.rand(1, 9999).'.'.$galune;
     					} 
     				 }
						@mkdir($temp,777);
						$tmpfile=$temp.$name;
				...
				foreach($object->parts as $obj)
					$this->walk($obj,&$msg,$main_content_type,&$plain,&$html);
			return $arr[]=array('papild'=>$msg,'plain'=>$plain,'html'=>$html);
		}
